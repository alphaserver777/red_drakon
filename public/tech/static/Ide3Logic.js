function Ide3Logic(gSpaceId, folderName, gUserId, browser, translate) {
  ...
  function AccessShower_AccessScreen_addUser(self, data) {
    ...
    machine.target = self;
    machine.existing = Utils.copyObject(self.access.roles[data]);
    browser.showAddUserScreen(machine);
  ...
  function AccessShower_AccessScreen_onData(self, data) {
    var admins = Object.keys(self.access.roles.admin);
    if (admins.length == 0) {
      browser.setAccessError(translate("ERR_NO_ADMINS"));
      self.state = "AccessScreen";
    } else {
      var change = buildAccessChange(self.oldAccess, self.access);
      if (change) {
        browser.showWorking();
        browser.sendPost("/api/multi_access", change, self);
  ...
  function AccessShower_AccessScreen_removeUser(self, data) {
    self.access.remove(data.role, data.user);
    self.state = "AccessScreen";
  }
  ...
  function AccessShower_AddingUser_cancel(self, data) {
    browser.showAccessScreen(self.access, null);
    self.state = "AccessScreen";
  }

  function AccessShower_AddingUser_onData(self, data) {
    self.access.add(self.role, data);
    browser.showAccessScreen(self.access, null);
    self.state = "AccessScreen";
  }
  ...
  function AccessShower_GettingAccess_onData(self, data) {
    browser.hideWorking();
    self.access = createAccess(self.spaceId, data);
    self.oldAccess = Utils.copyObjectDeep(self.access);
    browser.showAccessScreen(self.access, self);
    self.state = "AccessScreen";
  }
  ...
  function AccessShower_SavingAccess_onError(self, data) {
    ...
    if (data.error == "ERR_USER_LIMIT" && data.suggested) {
      browser.suggest(data.error, data.suggested, "access");
    } else {
      panic(data);
    }
    self.state = "AccessScreen";
  ...
  function AccessShower_Start_onData(self, data) {
    self.spaceId = data;
    var url = "/api/access/" + self.spaceId;
    browser.showWorking();
  ...
  function BuildManager_GettingModules_onData(self, msg) {
    var modules;
    browser.hideWorking();
    modules = msg.modules.map(makeModule);
    if (modules.length == 0) {
      browser.showNoModules(self);
      self.state = "Idle";
    } else {
      self.modules = Utils.listToMap(modules, "id");
      Utils.sort(modules, "name");
      browser.showChooseModule(modules, self);
  ...
  function BuildManager_Idle_buildModule(self, msg) {
    ...
    if (isHuman(current)) {
      browser.showWorking();
      url = "/api/prog_modules/" + current.spaceId;
      browser.sendGet(url, self);
  ...
  function DashboardShower_Start_onData(self, data) {
    startMachine(new SpacesLoader(), null, self);
  ...
  function DescriptionChanger_GetFolder_onData(self, data) {
    ...
    browser.showWorking();
    startMachine(new FolderGetter(), payload, self);
  ...
  function DescriptionChanger_ShowDialog_onData(self, data) {
    ...
    browser.showInputBox(true, title, old, onSave, null, false);
    self.state = "UserInput";
  }
  ...
  function DescriptionChanger_UserInput_onData(self, data) {
    ...
    backend
      .updateFolder(self.spaceId, self.folderId, payload)
      .then(self.onData)
      .catch(self.onError);
    self.state = "Saving";
  }
  ...
  function DiaPropChanger_GetProps_onData(self, data) {
    ...
    if (folder) {
      sendGetFolder(self.spaceId, self.folderId, self);
  ...
  function DiaPropChanger_SaveProps_onData(self, data) {
    browser.hideCentral();
    sendSaveDiagramProps(self.spaceId, self.folderId, data, self);
    self.state = "Saved";
  }
  ...
  function DiaPropChanger_SetModuleProps_onData(self, data) {
    browser.showChangeDiaProps(self, self.name, data, self.ro);
  ...
  function DiagramCreator_ChooseDiagramType_onData(self, data) {
    ...
    var parsed = parseId(self.parentId);
    self.spaceId = parsed.spaceId;
    self.parentFolder = parsed.folderId;
    folder = getFromCache(data.parentId);
    self.language = folder.language;
    browser.hideCentral();
    showCreateDialog(self.diagramType, self, folder.language);
  ...
  function handleDiagramCreationError(self, message) {
    browser.setInputBoxError(translate(message));
    browser.hideWorking();
    self.state = "EnterName";
    return;
  }
  ...
  function DiagramCreator_SendToServer_onData(self, data) {
    ...
    var id = makeId(self.spaceId, data.folder_id);
    goToFolder(id, onDone);
    self.state = "Done";
  }
  ...
  function DiagramSearch_Folders_onData(self, data) {
    ...
    browser.addToSearchList(found);
    startSearchItems(self.input, self, false);
  ...
  function DiagramSearch_Items_onData(self, data) {
    ...
    if (data.completed) {
      ...
      scheduleNextStateAfter(self, null, Config.SEARCH_PERIOD);
      self.state = "ItemsPause";
    }
  }
  ...
  function DiagramSearch_Start_onData(self, data) {
    startSearchFolders(self.input, self);
    self.state = "Folders";
  }
  ...
  function DrakonRenamer_SaveNewName_onData(self, data) {
    self.data = data;
    sendRename(self, self.data.spaceId, self.data.folderId, self.data.name);
  ...
  function DrakonRenamer_SavingChanges_onData(self, data) {
    ...
    if (globs.current.id == self.data.id) {
      cancelPolling();
      globs.saver = null;
      startMachine(new FolderShower(), self.data.id, self);
      self.state = "Reloaded";
    } else {
      complete(self, data);
      self.state = null;
  ...
  function DullSearch_Start_onInput(self, data) {
    ...
    browser.createSearchList(self.widgetId, defs, start);
    self.state = "Start";
  }
  ...
  function FolderCreatorGeneric_CreateMain_onData(self, data) {
    ...
    sendCreateDiagram(self.spaceId, self.id, props, self);
    self.state = "Expand";
  }
  ...
  function FolderCreatorGeneric_Expand_onData(self, data) {
    ...
    var machine, start;
    if (data.folder_id) {
      self.folderId = makeId(self.spaceId, data.folder_id);
    }
    machine = new TreeNodeExpander();
    start = {
      rowId: self.parentId,
    };
    startMachine(machine, start, self);
  ...
  function FolderCreatorGeneric_RefreshParent_onData(self, data) {
    ...
    getWidget("tree").expand(self.parentId);
    goToFolder(self.folderId, onDone);
    self.state = "Done";
  }
  ...
  function FolderCreatorGeneric_SaveProps_onData(self, data) {
    ...
    self.folderId = makeId(self.spaceId, data.folder_id);
    sendSaveFolderProps(self.spaceId, data.folder_id, self.props, self);
  ...
  function FolderCreator_CreateFolder_onData(self, data) {
    browser.hideCentral();
    self.props = data;
    startMachine(new FolderCreatorGeneric(), self, self);
    self.state = "Done";
  }
  ...
  function FolderCreator_EnterName_onData(self, data) {
    ...
    folder = getFromCache(data.parentId);
    browser.hideCentral();
    showCreateDialog(self.folderType, self, folder.language);
    if (self.folderType == "module") {
      self.state = "SetModuleProps";
    } else {
      self.state = "CreateFolder";
  ...
  function FolderCutterDeleter_GettingHistory_onData(self, data) {
    resetCutFolders();
    var currentDeleted = deleteFoldersFromUi(self.folders);
    if (self.cut) {
    ...
    if (currentDeleted) {
      goToFolder(self.parentId, self.onData);
    } else {
  ...
  function FolderCutterDeleter_RunningOperation_onData(self, data) {
    ...
    startMachine(new RecentGetter(), null, self);
  ...
  function FolderCutterDeleter_Start_onData(self, data) {
    ...
    self.cut = data.cut;
    if (self.cut) {
      type = getFolderCategory(data.folders[0]);
      globs.clipboard.copyToClipboard("cut-" + type, data.folders);
      scheduleNextState(self, null);
  ...
  function FolderGetter_GettingFolder_onData(self, data) {
    CallTrace.add("on folder data", [self.id, self.src]);
  ...
  function FolderGetter_Start_onData(self, data) {
    ...
    backend
      .getFolder(self.ids.spaceId, self.ids.folderId)
      .then(self.onData)
      .catch(self.onError);
  ...
  function FolderShower_GettingFolder_onData(self, data) {
    ...
    if (canContainChildren(globs.current)) {
      ...
      if (globs.current.type === "app") {
        setActiveScreen("middle_app", data.access);
        showApp(data);
        complete(self, data);
        self.state = null;
      } else {
  ...
  function FolderShower_GettingTheme_onData(self, data) {
    ...
    startMachine(new FontLoadingMachine(), self.diagram, self);
  ...
  function FolderShower_LoadingFonts_onData(self, data) {
    ...
    getEditor().resetMode();
    setActiveScreen("middle_diagram", self.diagram.access);
    setDiagram(self);
  ...
  function FolderShower_Start_onData(self, data) {
    ...
    startMachine(new FolderGetter(), getArgs, self);
    self.state = "GettingFolder";
  }
  ...
  function FontLoadingMachine_OnFont_onData(self, data) {
    if (hasMoreFontsToLoad(self)) {
      ...
      scheduleNextStateAfter(self, null, 2);
      self.state = "Sleeping";
    }
  }
  ...
  function FontLoadingMachine_Start_onData(self, data) {
    self.fonts = getEditor().getFonts(data);
    self.fontId = 0;
    if (hasMoreFontsToLoad(self)) {
      loadNextFont(self);
      self.state = "OnFont";
  ...
  function GenUrlShower_ShowUrl_onData(self, msg) {
    ...
    if (self.folder.mformat === "MES_PROGRAM") {
      url1 = "/gen/" + msg.gentoken + "/" + self.name + "/index.js";
      url2 = "/gen/" + msg.gentoken + "/" + self.name + "/package.json";
      url = [url1, url2];
    } else {
      url = ["/gen/" + msg.gentoken + "/" + self.name + ".js"];
    }
    browser.showUrlScreen(self.name, url, self);
    self.state = "Done";
  }
  ...
  function GoToFolderMachine_Start_onData(self, data) {
    self.id = data.id;
    startMachine(createFolderShower(true), data.id, self);
  ...
  function InputThrottle_Idle_onInput(self, data) {
    ...
    if (self.input) {
      if (self.input.length > 1) {
        self.timerId = browser.schedule(self, AddUserMs);
        self.state = "Waiting";
      } else {
        self.state = "Idle";
      }
  ...
  function InputThrottle_Waiting_onInput(self, data) {
    self.input = data.input;
    browser.clearTimeout(self.timerId);
    if (self.input) {
      if (self.input.length > 1) {
        self.timerId = browser.schedule(self, AddUserMs);
        self.state = "Waiting";
  ...
  function LoginMachine_Saving_onData(self, data) {
    ...
    hardGoToFolder(self.userId, data.diagram_id);
    self.state = null;
  }
  ...
  function ObjectCreator_ChooseObjectType_onData(self, msg) {
    ...
    target = makeTarget(function () {}, panic);
    data = {
      type: msg,
    ...
    startMachine(machine, data, target);
    self.state = null;
  }
  ...
  function ObjectCreator_Start_onData(self, msg) {
    var choice;
    self.parentId = getCurrentParent(msg);
    if (self.parentId) {
      choice = getChildObjectTypes(self.parentId);
      if (choice.length == 0) {
        self.state = null;
      } else {
        browser.showChooseObjectTypeDialog(self, choice);
  ...
  function OwnSaver_Dialog_login(self, data) {
    startMachine(new LoginMachine(), null, self);
    self.state = "Subdialog";
  }

  function OwnSaver_Dialog_signup(self, data) {
    startMachine(new SignupMachine(), null, self);
    self.state = "Subdialog";
  }
  ...
  function Paster_GettingHistory_onData(self, data) {
    ...
    if (self.parentId == globs.current.id) {
      startMachine(new FolderShower(), self.parentId, self);
    } else {
      payload = {
        id: self.parentId,
        src: "Paster",
  ...
  function Paster_Start_onData(self, data) {
    ...
    if (
      data.operation == "move" &&
      differentProjects(data.folders[0], data.parentId)
    ) {
      runManyOperation(data.parentId, data.folders, "copy", self);
      self.state = "RunningOperation";
    } else {
      runManyOperation(data.parentId, data.folders, data.operation, self);
      self.state = "RunningOperation";
    }
  }
  ...
  function ProjectCreator_Creating_onError(self, data) {
    ...
    if (data.error == "ERR_SPACE_LIMIT" && data.suggested) {
      browser.hideInputBox();
      browser.suggest(data.error, data.suggested, "createProject");
    } else {
      var message = makeErrorMessage(data);
      browser.setInputBoxError(message);
    }
  ...
  function ProjectCreator_EnterName_onData(self, data) {
    ...
    browser.sendPost("/api/space", payload, self);
    self.state = "Creating";
  }
  ...
  function ProjectDeleter_Sure2_onData(self, data) {
    browser.hideCentral();
    browser.showWorking();
    browser.sendDelete("/api/space/" + self.spaceId, self);
    self.state = "Deleting";
  }
  ...
  function ProjectSaver_BuildingZip_onData(self, data) {
    browser.hideWorking();
    browser.downloadFile(data.url, data.filename);
    complete(self, data);
    self.state = null;
  }
  ...
  function ProjectSaver_Start_onData(self, data) {
    self.spaceId = data;
    browser.showSaveProjectScreen(self.spaceId, self);
  ...
  function PropChanger_GetProps_onData(self, data) {
    ...
    self.ro = isReadonlyAccess(folder);
    if (folder) {
      sendGetFolderProps(self.spaceId, self.folderId, self);
      self.state = "SetModuleProps";
    } else {
      self.state = null;
    }
  ...
  function PropChanger_SaveModuleProps_onData(self, data) {
    browser.hideCentral();
    sendSaveFolderProps(self.spaceId, self.folderId, data, self);
    self.state = "Done";
  }
  ...
  function ReferencesSearch_Start_onData(self, data) {
    startSearchItems(self.input, self, true);
    self.state = "ItemsPause";
  }
  ...
  function Renamer_EnteringName_onData(self, data) {
    ...
    if (self.type == "drakon" || self.type == "app" || self.type == "mind") {
      CallTrace.add("rename diagram", [self.id, data]);
      var payload = {
        name: data,
        id: self.id,
      ...
      startMachine(new DrakonRenamer(), payload, self);
    } else {
      CallTrace.add("rename folder", [self.id, data]);
      sendRename(self, self.spaceId, self.folderId, data);
    }
  ...
  function Renamer_SendingToServer_onData(self, data) {
    ...
    browser.hideWorking();
    browser.hideInputBox();
    renameEverywhere(self.id, self.name);
    complete(self, null);
    self.state = null;
  }
  ...
  function Restorer_Start_onData(self, data) {
    self.id = data;
    var parsed = parseId(self.id);
    var url = "/api/restore/" + parsed.spaceId + "/" + parsed.folderId;
    browser.sendPost(url, {}, self);
  ...
  function RootFolderShower_ShowingSpaces_onData(self, data) {
    startMachine(new GoToFolderMachine(), { id: self.id }, self);
    self.state = "ShowingFolder";
  }
  ...
  function RootFolderShower_Start_onData(self, data) {
    self.id = data.id;
    startMachine(new SpacesLoader(), self.id, self);
  ...
  function RootTrashShower_ShowingSpaces_onData(self, data) {
    startMachine(new TrashLoader(), null, self);
  ...
  function Sharer_UserInput_access(self, data) {
    var ids = parseId(globs.current.id);
    showAccessScreen(ids.spaceId, self);
    self.state = "Access";
  }
  ...
  function ThrowTrash_GetAccount_onData(self, data) {
    browser.sendGet("/api/account", self);
    self.state = "GettingAccount";
  }
    ...
    if (data.rowId == "dashboard") {
      ...
      if (data.rowId == "trash") {
        ...
        if (data.rowId == globs.current.id) {
          ...
          self.start = data;
          startMachine(createFolderShower(true), data.rowId, self);
          self.state = "ShowingFolder";
        }
      }
    }
  ...
  function TryMeLoader_LoadingExample_onData(self, data) {
    self.folder = data;
    startMachine(new FontLoadingMachine(), self.folder, self);
    self.state = "LoadingFonts";
  }
  ...
  function TryMeLoader_LoadingFonts_onData(self, data) {
    browser.scheduleNextStateAfter(self, null, 100);
    self.state = "Sleeping";
  }
  ...
  function TryMeLoader_Sleeping_onData(self, data) {
    ...
    getEditor().setDiagram(self.folder, true);
  ...
  function TryMeLoader_Start_onData(self, data) {
    ...
    browser.sendGet(exampleUrl, self);
  ...
  function UserAdder_Primary_onData(self, data) {
    ...
    var toShow = Utils.subtract(found, self.existing);
    toShow.sort();
  ...
  function accountButton(evt, type, widget, id, cellId) {
    ...
    items.push(item);
    makeTextListItem(items, "Logout", browser.logout);
    browser.showPopupList(widget.id, items);
  }
  ...
  function addBuild(items, folder) {
    if (
      folder.type == "module" &&
      !(folder.language == "LANG_HUMAN") &&
      !isReadonlyAccess(folder)
    ) {
      makeTextListItem(
        items,
        "MES_BUILD",
  ...
  function addGetUrl(items, folder) {
    if (folder.type == "module" && !(folder.language == "LANG_HUMAN")) {
      makeTextListItem(items, "MES_GET_URL", function () {
        getGenUrl(folder.id);
      });
    }
  }
  ...
  function addNewItems(parentId, list) {
    ...
    if (types.length === 0) {
      ...
      while (true) {
        ...
        makeTextListItem(list, item.text, op, null, item.image);
        _ind8628++;
      }
      makeSeparator(list);
    }
  ...
  function addPathStage(name, id, isHyper) {
    ...
    while (true) {
      ...
      widget.addStage(name, id, isHyper);
      _ind998++;
    }
  }
  ...
  function addToCache(id, data, access) {
    ...
    if (folder.parent) {
      folder.parent = makeId(folder.space_id, folder.parent);
    }
    if (folder.children) {
      var _ind3569 = 0;
      ...
      while (true) {
        ...
        addToCache(makeId(child.space_id, child.id), child, folder.access);
        _ind3569++;
      }
    }
  ...
  function addToRecent(id, data) {
    ...
    index = find(recent, sameId);
    if (index == -1) {
    } else {
      removeAt(recent, index);
    }
  ...
  function bindEditor() {
    ...
    if (globs.wide && !globs.isTryMe) {
      openLeft = editor.createLeftButton(openLeftPane, "right", "#E2EDF5");
      openRight = editor.createRightButton(openRightPane, "left", "#E2EDF5");
      globs.openLeftIndex = openLeft;
      globs.openRightIndex = openRight;
      editor.hideLeftButton(globs.openLeftIndex);
      editor.hideRightButton(globs.openRightIndex);
    }
  }
  ...
  function canPasteFolder() {
    if (isReadonly()) {
      ...
      return canPasteInto(globs.current.id);
    }
  }
  ...
  function changeDescription(id) {
    ...
    startMachine(new DescriptionChanger(), id, target);
  }

  function changeDiagramProperties(id) {
    id = id || globs.current.id;
    startMachine(new DiaPropChanger(), id, null);
  }

  function changeFolderProperties(id) {
    startMachine(new PropChanger(), id, null);
  }
  ...
  function clearRubbish() {
    ...
    startAsync(new ThrowTrash(), null, null);
  }
  ...
  function closeLeftPaneCore(noSave) {
    ...
    if (globs.openLeftIndex == null) {
      ...
      getEditor().showLeftButton(globs.openLeftIndex);
    }
  ...
  function closeRightPaneCore(noSave) {
    ...
    if (globs.openRightIndex == null) {
      ...
      getEditor().showRightButton(globs.openRightIndex);
    }
    splitter = getWidget("splitter");
  ...
  function copyFoldersCore(selected) {
    ...
    if (selected.length == 0) {
      ...
      type = getFolderCategory(selected[0]);
      globs.clipboard.copyToClipboard("copy-" + type, selected);
      updateActionList();
    }
  }
  ...
  function createDiagramCore(parentId) {
    ...
    if (parentId) {
      ...
      target = makeTarget(function () {}, panic);
      startMachine(new DiagramCreator(), data, target);
    }
  }

  function createFolder() {
    createFolderCore(null, "folder");
  }
  ...
  function createModule() {
    createFolderCore(null, "module");
  }

  function createObjectCore(parentId) {
    var machine;
    machine = new ObjectCreator();
    startMachine(machine, parentId, null);
  }

  function createProject() {
    startMachine(new ProjectCreator(), null, null);
  }
  ...
  function createSearch(input) {
    ...
    if (_sw66160000_ === "middle_folder" || _sw66160000_ === "middle_app") {
      ...
      browser.createSearchList(defs, start);
  ...
  function cutFoldersCore(parentId, selected) {
    var data;
    CallTrace.add("cut folders", [selected]);
    if (selected.length == 0) {
    } else {
      data = {
        parentId: parentId,
      ...
      startAsync(new FolderCutterDeleter(), data, null);
    }
  }
  ...
  function deleteFoldersCore(parentId, selected) {
    var data;
    CallTrace.add("delete folders", [selected]);
    if (selected.length == 0) {
    } else {
      data = {
        parentId: parentId,
  ...
  function deleteFoldersFromUi(folders) {
    var currentDeleted = isAnyCurrent(folders);
    if (currentDeleted) {
    } else {
      updateActionList();
    }
  ...
  function deleteProject(spaceId) {
    var target;
    target = makeTarget(function () {}, panic);
    startMachine(new ProjectDeleter(), spaceId, target);
  }
  ...
  function expandMany(self) {
    ...
    while (true) {
      ...
      if (tree.hasItem(current)) {
        ...
        startMachine(new FolderGetter(), args, self);
        return false;
      }
      self.current++;
    }
  ...
  function exportButton(evt, type, widget, id, cellId) {
    var items;
    items = [];
    makeTextListItem(items, "MES_SAVE_AS_PNG", function () {
      browser.saveAsPng(1);
    });
    makeTextListItem(items, "MES_SAVE_AS_PNG_HI", function () {
      browser.saveAsPng(2);
    });
    makeTextListItem(items, "MES_SAVE_AS_SVG", browser.saveAsSvg);
    browser.showPopupList(widget.id, items);
  }
  ...
  function findItems(input) {
    ...
    diagram = getDiagram();
    scanItems(diagram.nodes, needle, items, cached.name, id, path);
    scanItems(diagram.free, needle, items, cached.name, id, path);
    return items;
  }
  ...
  function getActionList(checked) {
    ...
    items = [];
    if (checked.length > 0) {
      makeTextListItem(items, "MES_COPY", copyFolders);
      if (isReadonly()) {
      } else {
        makeTextListItem(items, "MES_CUT", cutFolders);
        if (canPasteFolder()) {
          makeTextListItem(items, "MES_PASTE", pasteFolders);
        }
        makeSeparator(items);
        makeTextListItem(items, "MES_DELETE", deleteFolders);
        if (checked.length == 1) {
          makeSeparator(items);
          makeTextListItem(items, "MES_RENAME", renameCheckedFolder);
        }
      }
    } else {
      if (canPasteFolder()) {
    ...
    if (checked.length == 1) {
      ...
      if (folder.type == "module") {
        makeSeparator(items);
        addGetUrl(items, folder);
        addBuild(items, folder);
        makeTextListItem(items, "MES_PROPERTIES", function () {
          changeFolderProperties(checked[0]);
        });
      } else {
        if (folder.type == "drakon") {
          makeTextListItem(items, "MES_DESCRIPTION", descriptionCheckedFolder);
          makeSeparator(items);
          makeTextListItem(items, "MES_PROPERTIES", function () {
    ...
    if (isReadonly()) {
      ...
      makeSeparator(items);
      makeTextListItem(items, "MES_NEW_OBJECT", create);
    }
    return items;
  }
  ...
  function getChildObjectTypes(folderId) {
    ...
    if (folder) {
      ...
      if (_sw73320000_ === "folder") {
        if (hasModuleAbove(folder)) {
          result.push(makeChoiceDiagram());
          result.push(makeChoiceFolder());
        } else {
          result.push(makeChoiceModule());
          result.push(makeChoiceApp());
          result.push(makeChoiceFolder());
        }
      } else {
  ...
  function getGenUrl(id) {
    startMachine(new GenUrlShower(), id, null);
  }
  ...
  function goToFolder(id, onCompleted) {
    ...
    if (
      id === globs.current.id &&
      !(globs.current.type === "folder") &&
      !(globs.current.type === "module") &&
      !(globs.current.type === "app")
    ) {
      if (onCompleted) {
      ...
      startAsync(new GoToFolderMachine(), start, onCompleted);
    }
  }
  ...
  function goToTrash(onCompleted) {
    willChangeScreen();
    startAsync(new TrashLoader(), null, onCompleted);
  }
  ...
  function init() {
    ...
    browser.initControls(globs.wide, globs.isTryMe);
    bindEditor();
    resetSearch();
  ...
  function initTryMe(target) {
    if (gUserId) {
      ...
      startMachine(new TryMeLoader(), null, target);
    }
  }
  ...
  function loadNextFont(self) {
    ...
    self.fontId++;
    browser.loadFontCore(font, self);
  }
  ...
  function loadSplitterValues() {
    ...
    if (globs.isTryMe || !globs.wide) {
      ...
      if (widget.leftVisible) {
        ...
        editor.showLeftButton(globs.openLeftIndex);
      }
      if (widget.rightVisible) {
        ...
        editor.showRightButton(globs.openRightIndex);
      }
    }
  }
  ...
  function makeIdFromChild(folder) {
    return makeId(folder.space_id, folder.id);
  }
  ...
  function makeRecentObject(folder) {
    var id;
    id = makeId(folder.space_id, folder.folder_id || folder.id);
    return {
      id: id,
      text: folder.name,
      type: folder.type,
  ...
  function makeSure1(spaceId) {
    var text = translate("MES_SURE_DELETE_SPACE") + " " + spaceId + "?";
    var titleLabel = {
      type: "wlabel",
  ...
  function onActionsClick(evt, type, widget, id, cellId) {
    ...
    folders = getWidget("folder_grid");
    status = folders.getChecked();
    items = getActionList(status.checked);
    if (items.length) {
      browser.showPopupList(widget.id, items);
    }
  }
  ...
  function onDiagramError(data) {
    ...
    if (saver) {
      if (data.error == "ERR_MODIFIED") {
        browser.showNotification(translate("ERR_MODIFIED"));
        saver.notSaved();
      } else {
        saver.onError(data);
      }
  ...
  function onEvent(evt, type, widget, rowId, cellId) {
    ...
    handler = getHandler(globs.handlers, widget, type);
    if (handler) {
      handler(evt, type, widget, rowId, cellId);
    } else {
      ctr = getHandler(globs.machineHandlers, widget, type);
      if (ctr) {
        start = {
          evt: evt,
          type: type,
  ...
  function onFolderGridContext(evt, widget, id) {
    ...
    if (shouldFindReferences(folder.type)) {
      makeTextListItem(list, "MES_FIND_REFERENCES", function () {
        findReferences(folder.name);
      });
      makeSeparator(list);
    }
    makeTextListItem(list, "MES_COPY", function () {
      copyFoldersCore(toCopy);
    });
    if (isReadonly()) {
    } else {
      makeTextListItem(list, "MES_CUT", function () {
        cutFoldersCore(parent, toCopy);
      });
      if (canPasteFolder()) {
        makeTextListItem(list, "MES_PASTE", pasteFolders);
      }
      makeSeparator(list);
      makeTextListItem(
        list,
        "MES_DELETE",
        function () {
          deleteFoldersCore(parent, toCopy);
        },
        null,
        "delete.png",
      );
      makeSeparator(list);
      makeTextListItem(list, "MES_RENAME", function () {
        rename(id);
      });
      makeTextListItem(list, "MES_DESCRIPTION", function () {
        changeDescription(id);
      });
      makeSeparator(list);
      if (folder.type == "module") {
        addGetUrl(list, folder);
        addBuild(list, folder);
        makeTextListItem(list, "MES_PROPERTIES", function () {
          changeFolderProperties(id);
        });
        makeSeparator(list);
      } else {
        if (folder.type == "drakon") {
          makeTextListItem(list, "MES_PROPERTIES", function () {
            changeDiagramProperties(id);
          });
          makeSeparator(list);
        }
      }
  ...
  function onFoldersContext(evt, type, widget, id, cellId) {
    if (id) {
      onFolderGridContext(evt, widget, id);
    } else {
      onFolderGridContextBack(evt, widget);
    }
  }
  ...
  function onPasteError(data) {
    ...
    if (data.error == "ERR_DIAGRAM_LIMIT") {
      browser.suggest(data.error, "team");
    } else {
      panic(data);
    }
  }
  ...
  function onProjectClick(evt, type, widget, id, cellId) {
    ...
    if (cellId == 2) {
      spacesWidget.mark(id);
      showProjectMenu(evt.clientX, evt.clientY, id);
    } else {
      goToFolder(id);
    }
  }
  ...
  function onProjectContext(evt, type, widget, id, cellId) {
    ...
    if (id) {
      ...
      if (FEATURE_SAVE_PROJECT && !isReadonly) {
        makeSeparator(list);
        makeTextListItem(list, "MES_LOAD_FROM_FILE", function () {
          showLoadFromFile(spaceId);
        });
        makeTextListItem(list, "MES_SAVE_TO_FILE", function () {
          showSaveToFile(spaceId);
        });
      }
      if (isAdmin) {
        spaceId = parseId(id).spaceId;
        makeSeparator(list);
        makeTextListItem(list, "MES_ACCESS", function () {
          showAccessScreen(spaceId, null);
        });
      }
      showContextMenu(evt, list);
    }
  ...
  function onSearchItem(type, target) {
    if (type === "folder") {
      goToFolder(target, null);
    } else {
      if (type === "item") {
        goToItem(target.id, target.itemId);
      }
    }
  }
  ...
  function onTrashClick(evt, type, widget, id, cellId) {
    if (cellId == 2) {
      startAsync(new Restorer(), id, null);
    }
  }

  function onTreeContext(evt, type, widget, id) {
    if (id) {
      onTreeContextFolder(evt, widget, id);
    } else {
      if (globs.user.userId) {
        onTreeContextBack(evt, widget);
      }
    }
  }
  ...
  function onTreeContextFolder(evt, tree, id) {
    ...
    CallTrace.add("onTreeContextFolder", [id]);
    tree.mark(id);
    list = [];
    if (id == "trash") {
      makeTextListItem(list, "MES_CLEAR_TRASH", clearRubbish);
    } else {
      if (id == "dashboard") {
      } else {
        folder = getFromCache(id);
        ...
        if (isSpace) {
          ...
          if (isReadonly) {
            ...
            makeTextListItem(list, "MES_CUT", function () {
              cutFoldersCore(folder.parent, [id]);
            });
          }
        }
        if (isReadonly) {
          ...
          if (canPasteInto(into)) {
            makeTextListItem(list, "MES_PASTE", function () {
              pasteFoldersCore(into);
            });
          }
          if (isSpace) {
          } else {
            ...
            makeTextListItem(
              ...
              "MES_DELETE",
              function () {
                deleteFoldersCore(folder.parent, [id]);
              },
              null,
              "delete.png",
            );
  ...
  function openLeftPaneCore() {
    var splitter;
    getEditor().hideLeftButton(globs.openLeftIndex);
    splitter = getWidget("splitter");
  ...
  function openRightPaneCore() {
    var splitter;
    getEditor().hideRightButton(globs.openRightIndex);
    splitter = getWidget("splitter");
  ...
  function pasteFoldersCore(parentId) {
    ...
    if (parentId) {
      ...
      startMachine(new Paster(), data, target);
    }
  }
  ...
  function pushNavFolder(id, name) {
    ...
    ids = parseId(id);
    url = makeFolderUrl(ids.spaceId, ids.folderId);
    title = makeTitle(ids.spaceId, ids.folderId, name);
    pushState(data, title, url);
  }

  function pushState(onState, title, url) {
    Nav.pushState(onState, title, window.location.origin + url);
  }
  ...
  function reloadDiagram() {
    startAsync(createFolderShower(false), globs.current.id, null);
  }
  ...
  function rename(id) {
    startMachine(new Renamer(), id, null);
  }
  ...
  function requestHistory(target) {
    var data;
    backend.getHistory().then(target.onData).catch(target.onError);
  }
  ...
  function rub_getUrl(self) {
    var url = "/api/trash/" + self.spaces[self.current];
    return url;
  }
  ...
  function saveAccessData(access) {
    if (access.setPublicAccess) {
      ...
      while (true) {
        ...
        var id = _keys4292[_ind4292];
        var folder = _col4292[id];
  ...
  function saveChange(spaceId, folderId, change, target) {
    ...
    backend
      .updateFolder(spaceId, folderId, change)
      .then(target.onData)
      .catch(target.onError);
  }
  ...
  function saveDiagram() {
    startMachine(new SignupMachineShort(), null, self);
  }
  ...
  function saveNow(saver, data) {
    ...
    target = makeTarget(onDiagramSaved, onDiagramError);
    ids = parseId(id);
    saver.lastIo = getTime();
    saveChange(ids.spaceId, ids.folderId, data, target);
  }
  ...
  function saveTry(target) {
    ...
    browser.sendPost("/api/save_try", diagram, target);
  }

  function saveUserPropsInMem(settings) {
    Utils.copyFields(settings, globs.user.props);
  }
  ...
  function scanItems(collection, needle, items, name, id, path) {
    ...
    if (collection) {
      ...
      while (true) {
        ...
        var itemId = _keys6663[_ind6663];
        var node = _col6663[itemId];
        text = node.text;
        text2 = node.text2;
        found = Utils.findNormSubstring(text, needle);
        if (found) {
          item = {
            name: name,
            type: "item",
          ...
          found = Utils.findNormSubstring(text2, needle);
          if (found) {
            item = {
              name: name,
              type: "item",
  ...
  function scheduleBuildCheck(self) {
    scheduleNextMessage(self, "timeout", null, BuildCheck);
  }
  ...
  function scheduleNextMessage(machine, method, data, delay) {
    ...
    src = "Schedule next method: " + machine.type_name + "." + method;
    browser.setTimeout(delayed, delay, src);
  }

  function scheduleNextState(machine, data) {
    scheduleNextStateAfter(machine, data, 1);
  }
  ...
  function sendBuild(self) {
    var msg, options;
    msg = {};
    self.builder = startMachine(new Builder(), msg, self);
    options = {
      state: "working",
      module: folderName,
    };
  ...
  function sendDelete(folders, target) {
    var items = folders.map(idToServerFolder);
    var payload = {
      items: items,
      operation: "delete",
    };
  ...
  function sendGetFolderProps(spaceId, folderId, target) {
    var url;
    url = "/api/folder_props/" + spaceId + "/" + folderId;
  ...
  function sendRename(self, spaceId, folderId, name) {
    ...
    backend
      .updateFolder(spaceId, folderId, payload)
      .then(self.onData)
      .catch(self.onError);
  }
  ...
  function sendSaveFolderProps(spaceId, folderId, data, target) {
    var url;
    url = "/api/folder_props/" + spaceId + "/" + folderId;
    browser.sendPost(url, data, target);
  }
  ...
  function setDiagram(self) {
    ...
    getEditor().setReadonly(isReadonly(), onReadonly);
    normalizeDiagram(self.diagram);
    getEditor().setDiagram(self.diagram, true);
    if (self.home) {
  ...
  function setItemsInFolderGrid(rows) {
    ...
    if (isReadonly()) {
      ...
      if (
        globs.current.type == "module" ||
        getFolderCategory(globs.current.id) == "module-folder"
      ) {
        buttons.push({
          main: true,
    ...
    folders.setItems(Utils.copyObjectDeep(rows), buttons);
    var _ind1166 = 0;
    var _col1166 = rows;
  ...
  function setPath(path) {
    ...
    while (true) {
      ...
      addPathStage(getNormalName(step), id, i < path.length - 1);
      i++;
    }
  }
  ...
  function setTreeChildren(data) {
    ...
    tree.setChildren(id, items);
  }

  function share() {
    startMachine(new Sharer(), null, null);
  }
  ...
  function shouldShowDemo() {
    if (
      ...
      isDiagram() &&
      globs.wide &&
      (globs.current.type == "drakon" || globs.current.type == "mind")
    ) {
      return true;
    } else {
      return false;
  ...
  function showAccessScreen(spaceId, target) {
    if (target) {
      ...
      target = makeTarget(hideCentral, panic);
    }
    startMachine(new AccessShower(), spaceId, target);
  }
  ...
  function showContextMenu(evt, list) {
    ...
    if (list.length == 0) {
      ...
      browser.showContextMenu(evt.clientX, evt.clientY, menu, null);
    }
  }
  ...
  function showCreateDialog(type, target, language) {
    ...
    browser.showInputBox(true, title, "", onSave, validate, true);
  }
  ...
  function showDashboard() {
    ...
    clearPath();
    addPathStage(translate("MES_DASHBOARD"), "<root>", false);
    browser.setTitle(getAppName());
    selectTreeItem("dashboard");
    setActiveScreen("middle_dashboard", null);
    pushNavDashboard();
  }
  ...
  function showLoadProjectFromFileDialog() {
    ...
    startMachine(new ProjectLoader(), undefined, target);
  }
  ...
  function showLoadFromFile(spaceId, target) {
    ...
    startMachine(new ProjectLoader(), spaceId, target);
  }
  ...
  function showProjectMenu(x, y, id) {
    ...
    if (isAdmin) {
      ...
      makeTextListItem(list, "MES_DELETE_SPACE", function () {
        deleteProject(spaceId);
      });
    }
    browser.showPopupListAt(x, y, list);
  }
  ...
  function showRecent() {
    ...
    while (true) {
      ...
      textCell = makeTextCell(spaceId + ": " + item.text);
      row = {
        id: item.id,
    ...
    clearPath();
    addPathStage(translate("MES_RECENT"), "<root>", false);
    grid = getWidget("middle_recent");
    grid.setItems(rows);
    setActiveScreen("middle_recent", null);
    pushNavRecent();
  }
  ...
  function showRenameDialog(folder, onSave, language) {
    ...
    title = translate(title);
    browser.showInputBox(true, title, folder.name, onSave, validate, true);
  }
  ...
  function showSaveToFile(spaceId, target) {
    ...
    startMachine(new ProjectSaver(), spaceId, target);
  }
  ...
  function showSpacesInFolder() {
    ...
    clearPath();
    addPathStage(translate("MES_SPACES"), "<root>", false);
    spacesWidget.setItems(rows);
    markPreviousSpace(spaces);
    setActiveScreen("middle_spaces", null);
    pushNavSpaces();
  }
  ...
  function showSpacesInTree() {
    ...
    var tree = getWidget("tree");
    tree.setChildren(undefined, items);
    for (var spaceId in globs.user.spaces) {
      var space = globs.user.spaces[spaceId];
  ...
  function showTrash(items) {
    ...
    while (true) {
      ...
      id = makeId(item.space_id, item.id);
      leftIcon = makeImageCell(item.type);
      textCell = makeTextCell(item.space_id + ": " + item.name);
      button = makeTextCell(translate("MES_RESTORE"));
      row = {
        id: id,
    ...
    clearPath();
    addPathStage(translate("MES_TRASH"), "<root>", false);
    grid.setItems(rows);
    selectTreeItem("trash");
    setActiveScreen("middle_trash", null);
    pushNavTrash();
  }
  ...
  function spacesSearchItems(input) {
    ...
    while (true) {
      ...
      var spaceId = _keys6330[_ind6330];
      var space = _col6330[spaceId];
  ...
  function startAsync(machine, start, onCompleted) {
    ...
    target = makeTarget(onData, panic);
    browser.showWorking();
    startMachine(machine, start, target);
  }
  ...
  function startBuildAll(buildInfo) {
    ...
    self.builder = startMachine(new AllBuilder(), msg, undefined);
    options = {
      state: "working",
      module: buildInfo.name,
    };
  ...
  function startMachine(machine, data, target) {
    ...
    scheduleNextState(machine, data);
    return machine;
  }
  ...
  function startUserSearch(user, target) {
    ...
    browser.sendPost("/api/find_users", payload, target);
  }
  ...
  function sv_dashboard(target) {
    if (gUserId) {
      startMachine(new DashboardShower(), null, target);
    } else {
      logonFirst();
    }
  }
  ...
  function sv_folder(spaceId, folderId, target) {
    var data, id;
    id = makeId(spaceId, folderId);
    data = {
      id: id,
    };
    startMachine(new RootFolderShower(), data, target);
  }

  function sv_recent(target) {
    if (gUserId) {
      startMachine(new RootRecentLoader(), null, target);
    } else {
      logonFirst();
    }
  }
  ...
  function sv_spaces(target) {
    if (gUserId) {
      startMachine(new RootSpacesShower(), null, target);
    } else {
      logonFirst();
    }
  }
  ...
  function sv_trash(target) {
    if (gUserId) {
      startMachine(new RootTrashShower(), null, target);
    } else {
      logonFirst();
    }
  }
  ...
  function toAllItems(data, needle) {
    ...
    while (true) {
      ...
      found = Utils.findNormSubstring(it.text, needle);
      if (found) {
        id = makeId(it.space_id, it.folder_id);
        path = it.path.slice(0, it.path.length - 1);
        item = {
          name: it.name,
          type: "item",
  ...
  function updateHistoryList() {
    var items = getRecent();
    var items2 = sortAndMarkColors(items);
    var recent = getWidget("recent");
    recent.setItems(items2);
  }
  ...
  function sortAndMarkColors(items) {
    ...
    var idToOrdinal = {};
    list.forEach((item, i) => {
      idToOrdinal[item.id] = i;
    });
    var items2 = takeFirst(items, 20);
    items2.forEach((item) =>
      setItemBackground(item, idToOrdinal, items.length),
    );
    return items2;
  }

  function takeFirst(list, count) {
