...
function render() {
  ...
  Object.entries(data.diagram.items).filter(([, item]) => ['action','question'].includes(item.type)).forEach(([id, item]) => {
    ...
    button.innerHTML = `<span class="kind">#${id} · ${item.type}</span><br>${item.content}`;
  ...
  card.className = 'card';
  card.innerHTML = `
    <label>Текст блока</label><textarea data-content>${item.content}</textarea>
    <label>Исполнитель</label><input data-key="executor" value="${text(op.executor)}">
    <label>Команда</label><textarea data-key="command">${text(op.command)}</textarea>
    <label>Входы</label><input data-key="inputs" value="${text(op.inputs)}">
    <label>Доказательства</label><input data-key="evidence" value="${text(op.evidence)}">
    <label>Условие</label><textarea data-key="note">${text(op.note)}</textarea>`;
